...
app.post('/api/login', async (req, res) => {
    ...
    if(username && password) {
        db.all('SELECT * FROM users WHERE username = '+username.toString(), (rows, err) => {
            if(!rows || !rows[0]) return res.status(404).json({ message: 'Username does not exist.' });
